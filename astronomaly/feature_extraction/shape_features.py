...
def fit_ellipse(contour, image, return_params=False, filled=True):
    ...
    if filled:
        ...
        ellipse_arr = np.zeros([y_npix, x_npix], dtype=np.float64)
    else:
        thickness = 1
...
def get_ellipse_leastsq(contour, image):
    ...
    contour_arr = np.zeros([y_npix, x_npix], dtype=np.float64)
...
def check_extending_ellipses(img, threshold, return_params=False):
    ...
    blank_canvas = np.zeros((new_width, new_height), dtype=np.float64)
