...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("generate_images_process_log_2.log"),  # Main log file
        logging.StreamHandler()                                 # Console output
    ]
)

# Add a dedicated error log file
error_handler = logging.FileHandler("generate_images_error_log_2.log")  # Separate file for errors
...
# Define paths
pkl_file_path = '/home/grespanm/github/data/image_data.pkl'
df_sample_path = '/home/grespanm/github/data/big_dataframe.parquet'
hdf5_file_path = '/home/grespanm/github/data/dr4_cutouts_2.h5'
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logging.info(f"Generated cutout for KIDS_ID {kids_id}, Tile {kids_tile}")
        ...
        with h5py.File(hdf5_path, 'a', libver='latest') as hdf5_file:
            with tqdm(total=total_items, desc="Writing to HDF5") as pbar:
                while True:
                    ...
                    logging.debug(f"Writing image for {kids_id} to HDF5")
...
def process_in_parallel(df, hdf5_path, num_workers=4, chunk_size=100):
    ...
    existing_ids = get_existing_ids(hdf5_path)  # Load existing IDs once
...
if __name__ == "__main__":
    ...
    # Ensure HDF5 file exists with the correct version
    if not os.path.exists(hdf5_file_path):
        with h5py.File(hdf5_file_path, 'w', libver='latest') as f:
            pass  # Create an empty file with the latest superblock version
    ...
    process_in_parallel(
        ...
        chunk_size=100 # Tune chunk size for optimal memory usage
    )
